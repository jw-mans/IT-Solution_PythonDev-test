/**
 * Анимация изменения счетчика лайков/дизлайков.
 *
 * @param {HTMLElement} el - элемент счетчика.
 * @param {number} newValue - новое значение счетчика.
 */
function animateCounter(el, newValue) {
    el.style.transition = "opacity 0.3s ease";
    el.style.opacity = 0;

    setTimeout(() => {
        el.textContent = newValue;
        el.style.opacity = 1;
    }, 300);
}

/**
 * Устанавливает обработчики голосования для всех форм.
 *
 * Логика:
 * 1. Определяет, является ли форма лайком или дизлайком.
 * 2. Отправляет POST-запрос на соответствующий URL.
 * 3. Обновляет счетчики лайков и дизлайков с анимацией.
 * 4. Подсвечивает активную кнопку (красная) и сбрасывает предыдущую.
 */
function setupVoteHandlers() {
    document.querySelectorAll(".like-form, .dislike-form").forEach(form => {
        form.addEventListener("submit", async e => {
            e.preventDefault();

            const quoteId = form.dataset.id;
            const isLike = form.classList.contains("like-form");
            const csrf = form.querySelector("[name=csrfmiddlewaretoken]").value;

            const res = await fetch(isLike ? `/like/${quoteId}` : `/dislike/${quoteId}`, {
                method: "POST",
                headers: { "X-CSRFToken": csrf }
            });
            const data = await res.json();

            ["like", "dislike"].forEach(type => {
                const counter = document.querySelector(`.${type}-form[data-id='${quoteId}'] .${type}-count`);
                animateCounter(counter, data[type + "s"]);
                const btn = document.querySelector(`.${type}-form[data-id='${quoteId}'] button`);
                btn.style.backgroundColor = "#fff";
                btn.style.color = "#ff4e50";
            });

            if (isLike && data.likes > 0) {
                document.querySelector(`.like-form[data-id='${quoteId}'] button`).style.cssText = "background-color:#ff4e50;color:#fff";
            }
            if (!isLike && data.dislikes > 0) {
                document.querySelector(`.dislike-form[data-id='${quoteId}'] button`).style.cssText = "background-color:#ff4e50;color:#fff";
            }
        });
    });
}

document.addEventListener("DOMContentLoaded", () => {
    setupVoteHandlers();
});
